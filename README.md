# MCP SQL : ODBC, MySQL, Microsoft SQL Server
...
2. Structure :
   - **`defaults`** : connexion utilisée pour chaque `type` lorsque l’URL ne contient pas `name=` (ex. `"odbc": "flexi"`).
   - **`databases`** : pour chaque moteur (`odbc`, `mysql`, `mssql`), un objet **nom → paramètres** (DSN ou host/user/database, etc.).
...
- **ODBC** : installation en local sur un PC Windows avec driver ODBC (pcsoft.fr), Node.js.
- **MySQL / MSSQL** : Node.js ; accès réseau au serveur SQL ; variables d’environnement (voir `env.example`).

---

## ODBC : source de données ODBC

Installer le Driver ODBC pour ODBC (Windows)(https://download.windev.com/fr/download/neo/ODBC/2026.awp)
Ouvre `odbcad32.exe` → Sources de données système → Ajouter → driver ODBC :
...
(ou sans argument si `ODBC_DSN` est dans `.env`) — le script utilise **`tables()`** ODBC, pas `SELECT 1` (ODBC peut échouer sur `SELECT 1` à cause de la pseudo-table `###DUAL0###`).
...
3. **Droits fichiers** : le compte du pool IIS / du service doit lire le `.wdd` et le dossier des fichiers ODBC (même logique que pour WinDev en service).
...
```txt
https://mon-domaine.com/mcp?token=SECRET&type=odbc&name=ma_base
```
...
- `query` — **SELECT uniquement** ; limite : `TOP` (ODBC, MSSQL), `LIMIT` (MySQL) sauf si la requête contient déjà un `LIMIT`
...
Tunnel Cloudflare (option)
    │
    ▼
Express : /mcp?type=odbc|mysql|mssql&token=…&name=…&database=…
    │
    ▼
